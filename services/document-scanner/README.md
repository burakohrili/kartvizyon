...
- `APP_BASE_URL`: `https://app.kartvizyon.app`.
...
gcloud run deploy kartvizyon-document-scanner --source . --region europe-west1 --allow-unauthenticated --memory 1Gi --cpu 1 --concurrency 1 --max-instances 3 --timeout 120 --set-env-vars APP_BASE_URL=https://app.kartvizyon.app --set-secrets DOCUMENT_SCAN_SECRET=kartvizyon-document-scan-secret:latest
```
